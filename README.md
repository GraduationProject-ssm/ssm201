...
# 绪论
## 1.1 研究背景
当前社会各行业领域竞争压力非常大，随着当前时代的信息化，科学化发展，让社会各行业领域都争相使用新的信息技术，对行业内的各种相关数据进行科学化，规范化管理。这样的大环境让那些止步不前，不接受信息改革带来的信息技术的企业随时面临被淘汰，被取代的风险。所以当今，各个行业领域，不管是传统的教育行业，餐饮行业，还是旅游行业，医疗行业等领域都将使用新的信息技术进行信息革命，改变传统的纸质化，需要人手工处理工作事务的办公环境。软件信息技术能够覆盖社会各行业领域是时代的发展要求，各种数据以及文件真正实现电子化是信息社会发展的不可逆转的必然趋势。本小区疫情防控管理系统也是紧跟科学技术的发展，运用当今一流的软件技术实现软件系统的开发，让医生管理信息完全通过管理系统实现科学化，规范化，程序化管理。从而帮助信息管理者节省事务处理的时间，降低数据处理的错误率，对于基础数据的管理水平可以起到促进作用，也从一定程度上对随意的业务管理工作进行了避免，同时，小区疫情防控管理系统的数据库里面存储的各种动态信息，也为上层管理人员作出重大决策提供了大量的事实依据。总之，小区疫情防控管理系统是一款可以真正提升管理者的办公效率的软件系统。
## 1.2 目的和意义
信息数据的处理完全依赖人工进行操作，会耗费大量的人工成本，特别是面对大量的数据信息时，传统人工操作不仅不能对数据的出错率进行保证，还容易出现各种信息资源的低利用率与低安全性问题。更有甚者，耽误大量的宝贵时间，尤其是对信息的更新，归纳与统计更是耗财耗力的过程。所以电子化信息管理的出现就能缓解以及改变传统人工方式面临的处境，一方面可以确保信息数据在短时间被高效处理，还能节省人力成本，另一方面可以确保信息数据的安全性，可靠性，并可以实现信息数据的快速检索与修改操作，这些优点是之前的旧操作模式无法比拟的。因此小区疫情防控管理系统为数据信息的管理模式的升级与改革提供了重要的窗口。
## 1.3 论文结构安排
...
图3.2 程序登录流程图
### 3.3.3 信息添加流程分析
程序的添加功能就是提供给操作者录入信息的功能，不管是涉及到业主信息添加，还是其它功能模块涉及到的信息添加，程序的信息添加流程（如图3.3所示）都是一致的。程序都是先对操作者录入的数据进行判定，这个判定规则是一段提前编写完成的程序代码，当程序判定数据符合要求时，才会把操作者录入的数据登记在数据表里面，比如添加的业主信息，就会把新添加的业主信息写入业主信息的数据表文件里面。
...
# 4 系统设计
小区疫情防控管理系统的设计方案比如功能框架的设计，比如数据库的设计的好坏也就决定了该系统在开发层面是否高效，以及在系统维护层面是否容易维护和升级，因为在系统实现阶段是需要考虑用户的所有需求，要是在设计阶段没有经过全方位考虑，那么系统实现的部分也就无从下手，所以系统设计部分也是至关重要的一个环节，只有根据用户需求进行细致全面的考虑，才有希望开发出功能健全稳定的程序软件。
## 4.1 系统概要设计
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|2|daigou\_name|String|代购商品|是|
|3|yonghu\_id|Integer|发布业主|是|
|4|daigou\_number|Integer|需求数量|是|
|5|insert\_time|Date|发布时间|是|
|6|daigou\_content|String|详情信息|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|14|jiankangdakai\_name|String|姓名|是|
|15|sex\_types|Integer|性别|是|
|16|jiankangdakai\_tiwen|String|当天体温|是|
|17|keshou\_types|Integer|有无咳嗽|是|
|18|wuaichu\_types|Integer|有无外出|是|
|19|jiankangdakai\_didian|String|外出地点|是|
|20|binghuan\_types|Integer|是否接触病患|是|
|21|gaofengxian\_types|Integer|是否出入高风险区域|是|
|22|jiankangdakai\_photo|String|照片|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|24|jubao\_name|String|标题|是|
|25|jubaoren|String|举报人|是|
|26|beijubaoren|Date|被举报人|是|
|27|insert\_time|Date|举报时间|是|
|28|yiqingcelve\_content|String|举报原因|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|30|laifangren\_name|String|来访人员名称|是|
|31|laifangren\_mudi|String|来访目的|是|
|32|insert\_time|Date|来访时间|是|
|33|yiqingcelve\_content|String|详情信息|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|35|news\_name|String|公告名称|是|
|36|news\_types|Integer|公告类型|是|
|37|news\_photo|String|公告图片|是|
|38|insert\_time|Date|公告时间|是|
|39|news\_content|String|公告详情|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|41|xiaodu\_name|String|消毒区域|是|
|42|insert\_time|Date|消毒时间|是|
|43|xiaodu\_content|String|详情信息|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|45|yonghu\_id|Integer|出入业主|是|
|46|yezhuchuru\_mudi|String|出入目的|是|
|47|insert\_time|Date|出入时间|是|
|48|yezhuchuru\_types|Integer|出入类型|是|
|49|yiqingcelve\_content|String|详情信息|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|51|yiqingcelve\_name|String|策略标题|是|
|52|insert\_time|Date|发布时间|是|
|53|yiqingcelve\_content|String|详情信息|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|55|yiqingfenbu\_chengshi|String|城市(区)|是|
|56|yiqingfenbu\_shengshi|String|省市(区)|是|
|57|yiqingfenbu\_xinzheng|Integer|新增确诊|是|
|58|yiqingfenbu\_wuzhengzhuang|Integer|新增无症状|是|
|59|yiqingfenbu\_fengxina|Integer|风险区域|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|61|yonghu\_name|String|业主姓名|是|
|62|sex\_types|Integer|性别|是|
|63|yonghu\_id\_number|String|身份证号|是|
|64|yonghu\_phone|String|手机号|是|
|65|yonghu\_zhuzhi|String|住址|是|
|66|yonghu\_photo|String|照片|是|
...
图5.1 业主信息管理页面
### 5.2 疫情策略管理
如图5.2显示的就是疫情策略管理页面，此页面提供给管理员的功能有：查看已发布的疫情策略数据，修改疫情策略，疫情策略作废，即可删除。

![](/md/blog.011.png)

图5.2 疫情策略管理页面
### 5.3疫情分布管理
如图5.3显示的就是疫情分布管理页面，此页面提供给管理员的功能有：根据疫情分布进行条件查询，还可以对疫情分布进行新增、修改、查询操作等等。
